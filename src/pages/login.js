...
export default function LoginPage() {
  ...
  const spotifyLoginUrl = 'https://soundscape-backend-tc9o.onrender.com/login'; // Replace with your backend URL

  useEffect(() => {
    const userId = localStorage.getItem('userId');
    if (userId) {
      // Validate session with the backend
      fetch(`https://soundscape-backend-tc9o.onrender.com/api/validate-session?userId=${userId}`)
        .then((response) => {
          if (response.ok) {
